...
class SingleplayerSession:
    ...
    def spawn_ship(self, position: Collection[str], ship_type: str) -> None:
        ...
        if self._game.is_fleet_ready(self.player):
            ...
            if self._game.current_player is self.enemy:
    ...
    def spawn_enemy_fleet(self) -> None:
        for ship_type, _ in self.roster:
            position = self.autoplacer.place(ship_type)
            self._game.add_ship(self.enemy, position, ship_type)

        self.ee.emit("fleet_ready", player=self.enemy)

    def make_enemy_move(self) -> None:
        if self.salvo_mode:
            count = self.enemy.ships_alive
        else:
            count = 1
